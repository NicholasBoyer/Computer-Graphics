...
int main(int argc, char* argv[])
{
	...
	while (!glfwWindowShouldClose(gWindow))
	{
		...
		gDeltaTime = currentFrame - gLastFrame / camSens;
...
void URender()
{
	...
	//rotation = glm::rotate(1.4f, glm::vec3(1.0, 0.0f, 0.0f));
	//view = glm::translate(glm::vec3(0.0f, 0.0f, -12.0f)) * rotation;
	//*

	// Chooses projection type
	if (ortho) { // Orthographic projection
		gCamera.Position.x = 0;
		gCamera.Position.y = 0;
		gCamera.Front = glm::vec3(0.0, 0.0, -1.0);
	...
	glProgramUniform4f(gProgramId, objectColorLoc, 1.0f, 0.8f, 0.4f, 1.0f);
	...
	// 2. Rotate the object
	rotation = glm::rotate(1.57f, glm::vec3(0.0, -0.5f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-3.3f, 3.0f, 4.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glProgramUniform4f(gProgramId, objectColorLoc, 0.5f, 0.1f, 0.1f, 0.1f);
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.25f, 6.0f, 0.25f));
	// 2. Rotate the object
	rotation = glm::rotate(1.57f, glm::vec3(0.0, -0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(3.0f, 3.0f, 4.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
{
	if (yoffset == -1) {
		gCamera.MovementSensitivity -= 0.1;
	}
	if (yoffset == 1) {
		gCamera.MovementSensitivity += 0.1;
	}
	if (gCamera.MovementSensitivity < 0.5f) {
		gCamera.MovementSensitivity = 0.5f;
	}
	if (gCamera.MovementSensitivity > 3.0f) {
		gCamera.MovementSensitivity = 3.0f;
	}
}
